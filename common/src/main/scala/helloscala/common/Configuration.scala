...
import helloscala.exception.{HSConfigurationException, HSException}
 ...
 */
case class Configuration(private var _underlying: Config) {

  // TODO 基于 etcd 或 consul 进行重设
  //  underlying.withFallback()

  def underlying: Config = _underlying

  /**
   * 合并两个HlConfiguration
   */
  def ++(other: Configuration): Configuration = Configuration(other.underlying.withFallback(underlying))
   ...
   */
  def getOptional[A](path: String)(implicit loader: ConfigLoader[A]): Option[A] = readValue(path, get[A](path))
...
object Configuration {
  ...
  def configError(message: String, origin: Option[ConfigOrigin], me: Option[Throwable]): HSException = {
    ...
    me.map(e => new HSConfigurationException(msg, e))
      .getOrElse(new HSConfigurationException(msg))
  }
